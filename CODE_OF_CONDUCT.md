# Code of Conduct

This project welcomes contributors who work respectfully, assume good intent,
and keep discussion focused on improving the software and data quality.

## Expected Behavior

- Be kind, direct, and constructive.
- Respect different experience levels and language backgrounds.
- Keep financial-data privacy central in every discussion.
- Accept maintainer decisions about scope, safety, and source quality.

## Unacceptable Behavior

- Harassment, threats, insults, or discriminatory language.
- Publishing real IBANs, customer data, private contact details, or screenshots
  containing personal financial information.
- Pressuring maintainers to accept unsourced data.
- Publicly disclosing a security issue before maintainers have had time to
  respond.

## Enforcement

Maintainers may edit, hide, lock, or delete comments and issues that violate this
code. Repeated or severe violations may result in temporary or permanent loss of
participation privileges.
